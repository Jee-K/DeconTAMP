...
def solve_abstract(problem, constraints=PlanConstraints(), stream_info={}, replan_actions=set(),
                  ...
                  min_iterations=0, visualize=False, verbose=True, **search_kwargs):
    """
    ...
    :param min_iterations: the minimum number of search iterations before terminating with a solution
    ...
    while (not store.is_terminated() or num_iterations < min_iterations) and (num_iterations < max_iterations) and (complexity_limit <= max_complexity):
        num_iterations += 1
